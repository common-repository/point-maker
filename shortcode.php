...
function point_maker_do_shortcode( $atts , $content = null ){
	...
	if ( !file_exists( POINT_MAKER_DIR . 'type/' . basename($atts['type']) . '.php' ) ) return;
	...
	require_once POINT_MAKER_DIR . 'type/' . basename($atts['type']) . '.php';


	wp_enqueue_style('point_maker_base', POINT_MAKER_URI . 'css/base.min.css',array(),POINT_MAKER_VERSION);
	wp_enqueue_style('point_maker_type_' . $atts['type'], POINT_MAKER_URI . 'css/' . basename($atts['type']) . '.min.css',array('point_maker_base'),POINT_MAKER_VERSION);

	
	$content = do_shortcode( shortcode_unautop( $content ) );

	$type = 'point_maker_type_' . basename($atts['type']);
